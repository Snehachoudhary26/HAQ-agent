      ...
      <header className="bg-[#0A542E] text-white sticky top-0 z-50 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-3 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-2 text-sm text-white/90">
            <Link to="/" className="flex items-center gap-1.5 px-3 py-1.5 hover:text-white">
              <Home size={15} /> Home
            </Link>
            <Link to="/schemes" className="flex items-center gap-1.5 bg-white/15 rounded-lg px-3 py-1.5 font-semibold text-white">
              <ListChecks size={15} /> Schemes
            </Link>
            <Link to="/agent" className="flex items-center gap-1.5 px-3 py-1.5 hover:text-yellow-300">
              <MessageCircle size={15} /> Chat Agent
            </Link>
            <Link to="/notifications" className="flex items-center gap-1.5 px-3 py-1.5 hover:text-white">
      ...
      <main className="w-full max-w-7xl mx-auto px-4 sm:px-8 py-6 sm:py-8">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm sm:text-base font-medium mt-1 text-black/60">
              सरकारी योजनाएं · Browse all schemes and check what you qualify for
            </p>
          </div>
          <div className="bg-[#F0FDF4] border border-[#0A542E]/20 rounded-xl px-4 py-3 flex items-start gap-2.5 max-w-xs">
            <Sparkles size={18} className="text-[#0A542E] flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-sm font-bold text-black">AI Recommendation</p>
              <p className="text-xs text-black/60">Based on your profile, you may qualify for <span className="font-bold text-[#0A542E]">5 schemes</span></p>
            </div>
            <ChevronRight size={16} className="text-black/30 flex-shrink-0 mt-1" />
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <div className="flex-1 relative">
            ...
            />
            <Mic size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-black/30" />
          </div>
        ...
        <div className="space-y-3">
          ...
          {filtered.map((s) => {
            ...
            return (
              <div key={s.id} className="bg-white border border-black/5 rounded-2xl overflow-hidden transition-all hover:shadow-md relative">
                <button onClick={() => toggleSaved(s.id)} className="absolute top-4 right-4 text-black/30 hover:text-[#0A542E] z-10 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0A542E]/30 rounded-full p-1 transition-colors">
                ...
                <div className="px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-4">
                  <div className="flex items-start gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-black/70 mt-1">{s.summary}</p>
                      <p className="text-xs text-[#0A542E]/80 mt-0.5">{s.summaryHi}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4 sm:gap-6 flex-shrink-0 pl-16 sm:pl-0">
                    ...
                    <div className="text-sm text-black/70 hidden md:block w-36 flex-shrink-0">
                    ...
                    <button onClick={() => setExpandedId(expanded ? null : s.id)} className="text-black/40 hover:text-[#0A542E] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0A542E]/30 rounded-full p-1 transition-colors flex-shrink-0">
                ...
                {expanded && (
                  <div className="px-4 sm:px-6 pb-5 border-t border-black/5 pt-4 space-y-4">
                    <div>
                      <p className="text-xs font-bold text-black/50 uppercase tracking-wide mb-2">Who qualifies</p>
                      <p className="text-sm font-medium text-black rounded-lg px-4 py-3 border" style={{ background: "#F0FDF4", borderColor: "#0A542E30" }}>
                        {s.eligibilitySnap}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-bold text-black/50 uppercase tracking-wide mb-2 flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between bg-[#F0FDF4] border border-[#0A542E]/15 rounded-xl px-5 py-4 mt-6">
          <p className="text-sm font-semibold text-black">Never miss a scheme you're eligible for! <span className="text-black/50 font-normal">Get instant SMS & Email alerts.</span></p>
          <Link to="/notifications" className="bg-[#0A542E] text-white text-sm font-bold px-4 py-2 rounded-full flex items-center gap-1.5 whitespace-nowrap">
